...
#ifdef SIMULATE
#include <float.h>   // só para M_PI se precisar
#endif
...
#ifdef SIMULATE
/* ----------- Simulador de quadros ------------- */
void* serial_reader_thread(void* arg)
{
    (void)arg;                       // não usa porta serial
    const int    N       = 360;      // pontos por quadro
    const float  base    = 2000.0f;  // mm
    const float  amp     = 500.0f;   // mm
    const float  degStep = 360.0f / N;
    float        phase   = 0.0f;
    int          frameId = 0;
    ...
    while (1) {
        ...
        last_count = N;
        for (int i = 0; i < N; ++i) {
            float thetaDeg = i * degStep;
            float thetaRad = (thetaDeg + phase) * M_PI / 180.0f;
            last_points[i].angle    = thetaDeg;                       // graus
            last_points[i].distance = (uint16_t)(base + amp * sinf(thetaRad));
        }
        pthread_mutex_unlock(&frame_mutex);

        if (DEBUG) {
            printf("[Sim] Frame %d gerado (%d pts)\n", ++frameId, last_count);
            fflush(stdout);
        }

        phase += 2.0f;                 // “gira” 2° por quadro
        if (phase >= 360.0f) phase -= 360.0f;

        struct timespec slp = {0, 100 * 1000 * 1000}; // 100 ms
        nanosleep(&slp, NULL);
    }
    return NULL;                       // nunca chega aqui
}
#else
/* ----------- Leitor real da serial ------------- */
void* serial_reader_thread(void* arg) {
